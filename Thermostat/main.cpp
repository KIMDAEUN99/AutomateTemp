...
void publishData()
{
  ...
  display.drawString(80, 11, dht_buf);
  data["temperature"] = dht_buf;

  tgtT = map(encoderValue, 0, 255, 10, 50);
  sprintf(dht_buf2, "%2.1f", tgtT);
  data["target"] = dht_buf2;
  display.drawString(80, 21, dht_buf2);
  display.display();
...
void setup()
{
  ...
  display.drawString(30, 1, "IoT Thermostat");
  display.drawString(30, 11, "Current");
  display.drawString(30, 21, "Target");
